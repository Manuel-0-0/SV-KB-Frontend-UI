...
const Dashboard = () => {
  const navBars = [
    ...
    { name: "Profile", component: "", icon: <UserIcon className="h-5 w-5" /> },
  ];
  const [activeNav, setActiveNav] = useState(navBars[0]);
    ...
    <main class="bg-gray-100 dark:bg-gray-800 h-screen overflow-hidden relative">
      <div class="flex items-start justify-between">
        <div class="h-screen hidden lg:block shadow-lg relative w-80">
          <div class="bg-[#324299] h-full dark:bg-gray-700">
            ...
            <nav class="mt-6">
              <div>
                {navBars.map((navBar) => (
                  <button
                    class={`w-full text-white flex items-center pl-6 p-2 my-2 transition-colors duration-200 justify-start ${
                      navBar.name === activeNav.name &&
                      "border-l-[5px] border-white"
                    }`}
                    onClick={() => setActiveNav(navBar)}
                  >
                    {navBar.icon}
                    <span class="mx-2 text-sm font-normal">{navBar.name}</span>
                  </button>
                ))}
              </div>
            </nav>
